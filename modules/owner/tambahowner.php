...
if (isset($_POST['simpan'])) {
  ...
  if (mysqli_fetch_assoc($result)) {
    ...
    echo "<script>window.location='index.php?page=tambahowner'</script>";

    return false;
  }


  mysqli_query($db, "INSERT INTO owner(id_owner,nama,jk,agama,alamat,tmpt_lahir,tgl_lahir,no_hp) VALUES ('$id_owner','$nama','$jk','$agama','$alamat','$tmp_lahir','$tgl_lahir','$no_hp')");

  $pass = password_hash($pass, PASSWORD_DEFAULT);

  mysqli_query($db, "INSERT INTO users(id_user,nama,email,pasword,level) VALUES ('$id_owner','$nama','$email','$pass','Owner')");
        ...
        <form id="demo-form2" data-parsley-validate class="form-horizontal form-label-left" method="post">
          ...
          <div class="form-group">
            ...
            <div class="col-md-6 col-sm-6 col-xs-12">
              ...
              $sql1  = "SELECT max(id_owner) AS terakhirpas FROM owner";
              $hasil1  = mysqli_query($db, $sql1);
              $data1   = mysqli_fetch_assoc($hasil1);
              $lastid1 = $data1['terakhirpas'];
              $lastnourut1 = (int)substr($lastid1,4,1);
              $nexturut1   = $lastnourut1+1;
              $nextid1     = "OWN-".sprintf("%01s",$nexturut1);

              ?>
              <input type="text" name="id_owner" required="required" value="<?php echo $nextid1; ?>" class="form-control col-md-7 col-xs-12" readonly>
            </div>
          </div>
          <div class="form-group">
