      ...
      <input type="text" value="<?php echo date('d-m-Y'); ?>" style="width:80px;" readonly><br><br>
        <table id="example" class="table table-striped table-bordered">
          ...
          <tbody>
            ...
            $query  = mysqli_query($db,"SELECT kb.*,det_poly.kd_detpol,det_poly.layanan,pasien.nama_lengkap,pasien.jk FROM kb JOIN det_poly ON kb.kd_detpol=det_poly.kd_detpol JOIN pasien ON kb.no_rm=pasien.no_rm AND DATE(kb.tgl_pemeriksaan)='$date'  WHERE kb.keterangan='Telah Melakukan Pembayaran' OR kb.keterangan='Selesai Diperiksa'");
              ...
              while ($pecah = mysqli_fetch_assoc($query)) {
               ...
               <td>
                <?php $query1  = mysqli_query($db,"SELECT * FROM pembayaran WHERE id_pemeriksaan='".$pecah['id_pemeriksaan']."'");
                    ...
                    if ($hitung1>0) { ?>              
                  <a class="btn btn-xs btn-success" href="./modules/bayarkb/notakb.php?id_pemeriksaan=<?php echo $pecah['id_pemeriksaan']; ?>" target="_blank" data-toggle="tooltip" data-placement="bottom" title="Cetak Nota"><i class="fa fa-print"></i></a>
                  ...
                  <?php }else{ ?>
                  <a class="btn btn-xs btn-info" href="index.php?page=transkb&id_pemeriksaan=<?php echo $pecah['id_pemeriksaan'];?>" data-toggle="tooltip" data-placement="bottom" title="Bayar"><i class="fa fa-edit"></i></a>                  
                <?php } ?>
                </td>
